...
class RNN_RNN(BasicModule):
    def __init__(self, args, embed=None):
        ...
        D = args.embed_dim
        H = args.hidden_size
        S = args.seg_num  # 文の相対位置をスコア計算に取り入れる際に，Documentを何分割するか指定
        P_V = args.pos_num
        P_D = args.pos_dim
        self.abs_pos_embed = nn.Embedding(P_V, P_D)
        self.rel_pos_embed = nn.Embedding(S, P_D)
        self.embed = nn.Embedding(V, D, padding_idx=0)  # 単語id を埋め込みベクトルに変換するメソッド padding に割り当てた id を与える
        if embed is not None:
            self.embed.weight.data.copy_(embed)  # 埋め込み行列を渡す

        # 単語，文章毎のEncode する双方向GRU-RNN 層を定義, 出力次元はhidden_sizeの二倍に
    ...
    def forward(self, x, doc_lens):
        ...
        word_out = self.max_pool1d(x, sent_lens)  # 論文では avg_pool1d
        ...
        for index, doc_len in enumerate(doc_lens):
            ...
            for position, h in enumerate(valid_hidden):
                ...
                rel_index = int(round((position + 1) * 9.0 / doc_len))  # ドキュメントを9文章ずつに分割した際の位置？ 実際には seg_numで9 の部分は変わるべきでは？変えることを想定していないとも
