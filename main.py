...
parser.add_argument('-predict_dir', type=str, default='data/predict_source.json')
parser.add_argument('-ref', type=str, default='outputs/ref')
parser.add_argument('-hyp', type=str, default='outputs/hyp')
parser.add_argument('-pred', type=str, default='outputs/predict')
...
def predict():
    # create summary
    # source sentences should be preprocessed into json form doc:
    embed = torch.Tensor(np.load(args.embedding)['embedding'])
    with open(args.word2id) as f:
        word2id = json.load(f)
    vocab = utils.Vocab(embed, word2id)

    with open(args.predict_dir) as f:
        examples = [json.loads(line) for line in f]
    predict_dataset = utils.Dataset(examples[0])

    predict_iter = DataLoader(dataset=predict_dataset,
                           batch_size=args.batch_size,
                           shuffle=False)
    if use_gpu:
        checkpoint = torch.load(args.load_dir)
    ...
    doc_num = len(predict_dataset)
    time_cost = 0
    file_id = 1
    for batch in tqdm(predict_iter):
        features, doc_lens = vocab.make_features_predict(batch)
        t1 = time()
        if use_gpu:
            probs = net(Variable(features).cuda(), doc_lens)
        else:
        ...
        for doc_id, doc_len in enumerate(doc_lens):
            ...
            doc = batch['doc'][doc_id].split('\n')[:doc_len]
            hyp = [doc[index] for index in topk_indices]
            with open(os.path.join(args.pred, str(file_id) + '.txt'), 'w') as f:
                f.write('\n'.join(hyp))
            start = stop
            file_id = file_id + 1
    print('Speed: %.2f docs / s' % (doc_num / time_cost))
